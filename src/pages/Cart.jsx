...
const Cart = () => {
  ...
  const removeFromCart = async(id)=>{
    try {
      const response= await axios.post("http://localhost:5000/product/removeproductfromcart",
        {user_id:"65e4a52a6a691876ff3a2fea", product_id:id},
        {
          headers:{
            "Content-Type":"application/json"
          }
  ...
  useEffect(() => {
    ...
    userCart.forEach((item) => {
      cost +=
        ...
        (item.product.product_price * item.product.product_discount) / 100)* item.quantity;
    });
      ...
      <div className="w-full flex flex-col lg:flex-row gap-4">
        <div className="lsit w-full p-2 flex flex-col gap-y-2">
          {userCart.length>0 ? userCart?.map((item) => {
            ...
            return (
              <div className="cart_items w-full h-42 p-2 border-2 rounded-lg flex gap-2" key={item.product._id}>
                <img
                ...
                <div className="w-full flex flex-col">
                  ...
                  <div className="w-full flex gap-3 font-semibold text-cyan-600 mt-2">
                    <span className="cursor-pointer hover:text-cyan-800" onClick={()=>removeFromCart(item.product._id)}>
                      Remove
                    </span>{" "}
                    |
                    <Link to={`/porductwithcategory/${item.product.product_category}`}><span className="cursor-pointer hover:text-cyan-800">
                      See more
                    </span></Link>
                  </div>
                </div>
